  ...
  <head>
    ...
    <script type="text/javascript">
		...
		function del(id){
			...
			if(confirm("确定要删除行业类型？")){
         		$.post('<%=basePath%>admin/IndustryServlet?act=del&id='+id, function(data){
	          	data=JSON.parse(data);
	          	if(data[0]=="SUCCESS"){
	          		location.href="<%=basePath%>admin/IndustryServlet?act=list&lx=zda";
	          	}
	    		});
			}
		}
  ...
  <body class="mainbody">
  	<div id="lb">
        <div class="select_box">
            ...
            <span style="float:right;"><a href="#" onclick="location.href='IndustryServlet?act=list&lx=zda';" target="_self">刷新</a></span>
        </div>
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="msgtable">
            <tr>
              <th align="center">行业类型名称</th>
            ...
            <tr>
            	...
            	<c:forEach items="${IndustryList}" var="item1">
            	<c:if test="${item.id==item1.pid}">
	            <tr>
	                <td height="20" style="text-align:left;">&nbsp;┗&nbsp;${item1.mc}</td>
	                <td align="center">${item1.px}</td>
	                <td align="center">
	                    <a href="#%20" target="_self" onclick="edit('${item1.pid}','${item1.id}','${item1.mc}','${item1.px}');">修改</a>
	                    <a href="#%20" target="_self" onclick="del('${item1.id}');">删除</a>
	                </td>
	            </tr>
	            </c:if>
	            </c:forEach>
    ...
    <form name="frm" id="frm" method="post" target="_self" action="<%=basePath%>admin/IndustryServlet?act=add">
    	<div class="select_box">
            ...
            <input type="hidden" name="lx" value="zda" />
        </div>
