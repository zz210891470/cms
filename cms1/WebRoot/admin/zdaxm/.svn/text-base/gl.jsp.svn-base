...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <script type="text/javascript">
        ...
        //--------
        function edit(i){
            $("#theFrame").attr("src","<%=basePath%>admin/zdaxm/ZdaxmServlet?action=toEdit&id="+i);
        ...
        //添加列表项
        function add(){
            $("#theFrame").attr("src","<%=basePath%>admin/zdaxm/ZdaxmServlet?action=toAdd");
            $("#frameDiv").removeClass("hideDiv");
            $("#frameDiv").addClass("showDiv");
        }
      ...
      //浏览列表项
        function read(i){
            $("#theFrame").attr("src","<%=basePath%>admin/zdaxm/ZdaxmServlet?action=toRead&id="+i);
            $("#frameDiv").removeClass("hideDiv");
            $("#frameDiv").addClass("showDiv");
        }
        ...
        function hideAddFrame(){
             document.location.href='<%=basePath%>admin/zdaxm/ZdaxmServlet?action=list';
        }
        ...
        function del(i){
            ...
            document.all.pageform.action='<%=basePath%>admin/zdaxm/ZdaxmServlet?action=del'; 
            document.all.pageform.submit(); 
        }
        ...
        function goPage(i){
        	$("#pageNo").val(i);
      	    document.all.pageform.action='<%=basePath%>admin/zdaxm/ZdaxmServlet?action=list';
		    document.all.pageform.submit();    
        }
  ...
  <body class="mainbody">
  	<form name="pageform" method="post" action="" id="form">
    	<div class="navigation">首页 &gt; 重大项目管理 &gt; 项目信息管理</div>
    	    	
    	<div class="tools_box">
	    <div class="tools_bar">
            <div class="search_box">
			    <input name="searchStr" type="text" id="txtKeywords" class="txtInput" value="${searchStr }"/>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="msgtable">
			<tr>
			  ...
			  <th width="60" align="left">总投资</th>
			  <th width="60" align="left">计划投资</th>
			...
			<c:forEach items="${pager.resultList}" var="item">
		     <tr>
		        ...
		        <td>${item.nd }</td>
		        <td style="text-align:left;" align="center">${item.mc }</td>
