    ...
    .hideDiv{
      ...
      <script type="text/javascript">
        ...
        function listrelease(i){
            document.all.pageform.action='<%=basePath%>admin/article/SubArticleServlet?act=listrelease&id='+i;     
         ...
         function recycle(){
        	...
        	$("#theFrame").attr("src",'<%=basePath%>admin/article/SubArticleServlet?act=recyclepage&pid='+pid);
        ...
        //编辑列表项
        function edit(i){
            $("#theFrame").attr("src","<%=basePath%>admin/article/SubArticleServlet?act=updatepage&id="+i);
            $("#frameDiv").removeClass("hideDiv");
            $("#frameDiv").addClass("showDiv");
        }
      ...
      function add(){
		    $("#theFrame").attr("src",'<%=basePath%>admin/article/SubArticleServlet?act=addpage&pid='+$('#pid').val());
            $("#frameDiv").removeClass("hideDiv");
            $("#frameDiv").addClass("showDiv");  
      }
      ...
      function gopage(i){
      	...
      	document.all.pageform.action='<%=basePath%>admin/article/SubArticleServlet?act=list&pageNo='+i;
		document.all.pageform.submit(); 
      }
        ...
        function setPageSize(){
	        ...
	        if( pageSize != "" && /^[0-9]*$/.test(pageSize) ){
	            $("#pageSize").val(pageSize);
	      	    document.all.pageform.action='<%=basePath%>admin/article/SubArticleServlet?act=list';
			    document.all.pageform.submit();
            }else{
		        alert("请输入数字！");
		    }   
            ...
            $("input[name='itemList']").each(function(){   
             ...
             if(num>0){
                 if(confirm("确定要删除选中项?")){
			         ...
			         document.all.pageform.action='<%=basePath%>admin/article/SubArticleServlet?act=deleteItems';     
				     document.all.pageform.submit();   
			     }
		     }else{
			     alert("请至少选中一项");
 ...
 /*    $(function(){
	    ...
	    if (key == 13) {
	       var pagesize=$('#txtPageNum').val();      
      	   document.all.pageform.action='<%=basePath%>admin/article/SubArticleServlet?act=list&pageNo=1&pageSize='+pagesize;
		   document.all.pageform.submit(); 
	    }
	    });
...
<form name="pageform"  method="post">
 ...
 <input type="button" name="btnSearch" value="搜 索" onclick="dosearch();" id="btnSearch" class="btnSearch" />
            ...
            <a  id="btnDelete" class="tools_btn" href="javascript:onDelete()"><span><b class="delete">批量删除</b></span></a>
