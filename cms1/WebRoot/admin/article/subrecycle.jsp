...
<script type="text/javascript" src="../js/function.js"></script>
    ...
    <style>
      ...
      function dosearch(){
         $("#pageNo").val("1");
         document.all.pageform.action='<%=basePath%>admin/article/SubArticleServlet?act=recyclepage';
        ...
        function setPageSize(){
	        ...
	        if( pageSize != "" && /^[0-9]*$/.test(pageSize) ){
	            $("#pageSize").val(pageSize);
	      	    document.all.pageform.action='<%=basePath%>admin/article/SubArticleServlet?act=recyclepage&pid='+pid;
			    document.all.pageform.submit();
            }else{
		        alert("请输入数字！");
		    }   
      ...
      function gopage(i){
      	...
      	document.all.pageform.action='<%=basePath%>admin/article/SubArticleServlet?act=recyclepage&pageNo='+i;
		document.all.pageform.submit(); 
      }
            ...
            $("input[name='itemList']").each(function(){   
             ...
             if(num>0){
                 if(confirm("确定要删除选中项?")){
			         ...
			         document.all.pageform.action='<%=basePath%>admin/article/SubArticleServlet?act=RdeleteItems';     
				     document.all.pageform.submit();   
			     }
		     }else{
			     alert("请至少选中一项");
            ...
            $("input[name='itemList']").each(function(){   
             ...
             if(num>0){
                 if(confirm("确定要恢复选中项?")){
			         ...
			         document.all.pageform.action='<%=basePath%>admin/article/SubArticleServlet?act=resume';     
				     document.all.pageform.submit();   
			     }
		     }else{
			     alert("请至少选中一项");
 ...
 /*    $(function(){
	    ...
	    if (key == 13) {
	       var pagesize=$('#txtPageNum').val();      
      	   document.all.pageform.action='<%=basePath%>admin/article/SubArticleServlet?act=recyclepage&pageNo=1&pageSize='+pagesize;
		   document.all.pageform.submit(); 
	    }
	    });
    ...
    <div class="tools_box">
	    <div class="tools_bar">
            <div class="search_box">
                ...
                <input type="button"  value="搜 索" onclick="dosearch();"  class="btnSearch" />
		    </div>
