			...
			if(num2 >0 ){
		        ...
		        if(contains(answerstr,";;",false)){
				   alert("问题下必须要有选项！");
					...
					document.all.dataform.action='PeopleVoteServlet?act=update';
...
<div class="navigation"><a href="javascript:back();" class="back">后退</a>首页 &gt; 公众参与管理 &gt; 网上调查管理</div>
    <div id="contentTab">
    <ul class="tab_nav">
        <li class="selected"><a onclick="tabs('#contentTab',0);" href="javascript:;">编辑网上调查</a></li>
        <li><a onclick="tabs('#contentTab',1);" href="javascript:;">编辑调查选项</a></li>
    </ul>
    ...
    <div class="tab_con" style="display:block;">
        <table class="form_table">
            ...
            <tr>
                <th>开始时间：</th>
                <td><input class="Wdate" type="text" onClick="WdatePicker()" name="kssj" value="${vote.kssj }"/><font color="red">&nbsp;*</font></td>
            </tr>
            <tr>
                <th>结束时间：</th>
                <td><input class="Wdate" type="text" onClick="WdatePicker()" name="jssj" value="${vote.jssj }"/><font color="red">&nbsp;*</font></td>
            </tr>
            <tr>
                <th>调查标题：</th>
                <td><input  type="text"  name="bt"  value="${vote.zt }"  /><font color="red">&nbsp;*</font></td>
            </tr>
            <tr>
                <th height="200">调查说明：</th>
                <td><textarea  cols="40" rows="8" name="nr" >${vote.nr }</textarea></td>
            </tr>
    ...
    <div class="tab_con">
    	...
    	<table class="form_table">
            <tr>
                <td id="point">
		           <c:forEach items="${voteItemList}" var="item" varStatus="status1">
		           <div id="questionarea${status1.count }" data-id="${status1.count }" data-type="${item.type}"><div><span id="name${status1.count }">
		               <c:if test="${item.type=='1'}">单选调查：</c:if>
		               <c:if test="${item.type=='2'}">多选调查：</c:if>
		             </span><input type="text" id="question${status1.count }" value="${item.wt }" />&nbsp;&nbsp;&nbsp;&nbsp;题目排序：<input name="numsort" type="text" id="sort${status1.count }" value="${item.sort }"/>
		             &nbsp;&nbsp;<input type="button" value="添加选项"  data-id="${status1.count }" onclick="addSingleAnswer(${status1.count })" />&nbsp;&nbsp;
		             <input type="button"  data-id="${status1.count }" value="删除此题" onclick="deletequestion(${status1.count })"  /></div>
                       <div style="padding-left:20px;" id="answer${status1.count }">
		              
		               <c:forEach items="${voteAnswersList}" var="answer" varStatus="status" >
		                  <c:if test="${item.type=='1'}">             
		                       <c:if test="${answer.wtid==item.id}">
		                       	
		                         <span><input type="radio" name="RadioGroup1" value="单选"  /><input type="text" value="${answer.da }"/></span><input type="button" value="删除此项" onclick="javascript:deleteItem(this)" /><br />
		                       </c:if>
		                  </c:if>
		                  <c:if test="${item.type=='2'}">     
		                       <c:if test="${answer.wtid==item.id}">
		                       <span><input type="checkbox" name="CheckboxGroup1" value="复选框"  /><input type="text"  value="${answer.da }"/></span><input type="button" value="删除此项" onclick="javascript:deleteItem(this)" /><br />
