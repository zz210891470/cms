...
<head>
	...
	<script type="text/javascript">
      function toSubColumn(id){
          ...
          if(data[0]=="SUCCESS"){
            alert("该栏目下存在文章，不能再添加栏目");  
          }else{
            document.location.href='<%=basePath%>admin/setting/SpecialColumnServlet?act=toSubColumn&pid='+id;
          }
          });
      }
      ...
      function returnuplevel(){
	      var id=$('#rid').val();  
	      if(id=="0"){
	        document.location.href='<%=basePath%>admin/setting/SpecialColumnServlet?act=list';
	      }else{
	        document.location.href='<%=basePath%>admin/setting/SpecialColumnServlet?act=toSubColumn&pid='+id;
	      }      
		  return false;   
      ...
      function add(){
       	var id=$('#pid').val(); 
		$.ligerDialog.open({ url: '<%=basePath%>admin/setting/SpecialColumnServlet?act=addSubPage&pid='+id,title: '添加栏目', width:500, height: 300, modal:false, isResize:true });
      }
      
      function gopage(i){
	    var pagesize=$('#txtPageNum').val();   
	     var id=$('#pid').val();    
      	document.location.href='<%=basePath%>admin/setting/SpecialColumnServlet?act=toSubColumn&pid='+id+'&pageNo='+i+'&pageSize='+pagesize;
		return false;
      }
      
      function edit(id){
      	$.ligerDialog.open({ url: 'SpecialColumnServlet?act=subupdatepage&id='+id,title: '编辑栏目', width:500, height: 300, modal:false, isResize:true });  
      }
      function del(id){
      	$.post('<%=basePath%>admin/article/SubArticleServlet?act=hasArticle&id='+id, function(data){
          	data=JSON.parse(data);
     ...
     $(function(){
	    ...
	    if (key == 13) {
	       var pagesize=$('#txtPageNum').val();     
	       var id=$('#pid').val(); 
      	   document.location.href='<%=basePath%>admin/setting/SpecialColumnServlet?act=toSubColumn&pid='+id+'&pageNo=1&pageSize='+pagesize;
		   return false;
	    }
	    });
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="msgtable">
     ...
     <tr>
        <td align="center" height="25">
          <span class="checkall">
          <input id="rptList_ctl01_chkId" type="checkbox" name="itemList" value="${column.id}"/>
          </span>
        </td>
        <td align="center">${column.id }</td>
        <td align="center">${column.lmmx }</td>
        <td align="center">${column.lmmc }</td>
