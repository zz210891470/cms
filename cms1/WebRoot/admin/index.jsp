  ...
  <div id="layout1" style="width:99.2%; margin:0 auto;  "> 
	<!--左边-->
        <div position="left"  title="管理菜单" id="accordion1"> 
        	<c:if test="${fn:contains(qx,'lm')}">
			<div title="栏目内容管理" class="l-scroll" iconcss="menu-icon-model" id="tab1">
				 <ul id="tree1" style="margin-top:3px;" class="ztree">
				 </ul>
			</div>
			</c:if>
			<c:if test="${fn:contains(qx,'zt')}">
			<div title="专题内容管理" class="l-scroll" iconcss="menu-icon-model" id="tab2">
				 <ul id="tree2" style="margin-top:3px;" class="ztree">
				 </ul>
			</div>
			</c:if>
			<c:if test="${fn:contains(qx,'@zf')}">
			<div title="政府信息公开专栏" iconcss="menu-icon-order" id="tab3">
			<div style=" height:7px;"></div>
				  <ul class="nlist">
				  <li><a class="l-link" href="javascript:f_addTab('xxgk_gkzn','信息开指南','infoopen/InfoGuideServlet?act=read&topic=gkzn')">政府信息公开指南</a></li>
				  <li><a class="l-link" href="javascript:f_addTab('xxgk_gkml','信息公开目录','infoopen/InfoOpenServlet?act=list')">政府信息公开目录</a></li>
				  <li><a class="l-link" href="javascript:f_addTab('xxgk_gkgd','信息公开规定','infoopen/InfoRegServlet?act=list')">政府信息公开规定</a></li>
				  <li><a class="l-link" href="javascript:f_addTab('xxgk_ysqgk','依申请公开','infoopen/InfoGuideServlet?act=read&topic=sqgk')">依申请公开</a></li>
				  <li><a class="l-link" href="javascript:f_addTab('xxgk_ndbg','信息公开年度报告','infoopen/InfoReportServlet?act=list')">政府信息公开年度报告</a></li>
				  </ul>
			</div> 
			</c:if>
			<c:if test="${fn:contains(qx,'@gz')}">
			<div title="公众参与管理" iconcss="menu-icon-member" id="tab4">
			<div style=" height:7px;"></div>
				<ul class="nlist">
				 <li><a class="l-link" href="javascript:f_addTab('mail','领导信箱管理','public/PeopleMailServlet?act=list')">领导信箱管理</a></li>
				 <li><a class="l-link" href="javascript:f_addTab('complain','咨询投诉管理','public/PeopleConsultServlet?act=list')">咨询投诉管理</a></li>
				 <li><a class="l-link" href="javascript:f_addTab('collect','民意征集管理','public/PeopleSuggestionServlet?act=list')">民意征集管理</a></li>
				 <li><a class="l-link" href="javascript:f_addTab('vote','网上调查管理','public/PeopleVoteServlet?act=list')">网上调查管理</a></li>
				 <li><a class="l-link" href="javascript:f_addTab('question','常见问题管理','public/PeopleQuestionServlet?act=list')">常见问题管理</a></li>
				</ul>
			</div>    
			</c:if>
			<c:if test="${fn:contains(qx,'@fz')}">
			<div title="辅助功能管理" iconcss="menu-icon-order" id="tab5">
			<div style=" height:7px;"></div>
				  <ul class="nlist">
				  <li><a class="l-link" href="javascript:f_addTab('banner','首页广告设置','help/AdManageServlet?act=showpage')">首页广告设置</a></li>
				  <li><a class="l-link" href="javascript:f_addTab('link','友情链接管理','help/UrlManageServlet?act=list&lb=1')">友情链接管理</a></li>
				  </ul>
			</div> 
			</c:if>
			...
			<div style=" height:7px;"></div>
				<ul class="nlist">
				  ...
				  <li><a class="l-link" href="javascript:f_addTab('backup','系统数据备份','setting/SysDataServlet?act=list')">系统数据备份</a></li>
				</ul>
			</div> 
			</c:if>
        </div>
