...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <script type="text/javascript">
        ...
        //浏览列表项
        function read(i){
            $("#theFrame").attr("src","<%=basePath%>admin/cbxm/CbxmServlet?action=toRead&id="+i);
            $("#frameDiv").removeClass("hideDiv");
            $("#frameDiv").addClass("showDiv");
        }
        ...
        //添加列表项
        function add(){
            $("#theFrame").attr("src","<%=basePath%>admin/cbxm/CbxmServlet?action=toAdd");
            $("#frameDiv").removeClass("hideDiv");
            $("#frameDiv").addClass("showDiv");
        }
        ...
        function hideAddFrame(){
             document.location.href='<%=basePath%>admin/cbxm/CbxmServlet?action=list';
        }
        ...
        function goPage(i){
        	$("#pageNo").val(i);
      	    document.all.pageform.action='<%=basePath%>admin/cbxm/CbxmServlet?action=list';
		    document.all.pageform.submit();    
        }
        ...
        function topublish(i){
        $.post("<%=basePath%>admin/cbxm/CbxmServlet?action=publish&id="+i,
        function(data){
             if(data.result==0){
             alert("发布成功");
             }else{
  ...
  <body class="mainbody">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="msgtable">
			<tr>
			    ...
			    <th width="120" align="left">申报单位</th>
			...
			<c:forEach items="${pager.resultList}" var="item">
		     <tr>
		        ...
		        <td align="center">
		          ...
		          <a href="javascript:topublish(${item.id })" >发布</a>
		          </td>
