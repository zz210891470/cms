...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <script type="text/javascript">
        ...
        //--------
        function edit(i){
            $("#theFrame").attr("src","<%=basePath%>admin/ManageServlet?action=toEdit&lx=cb&id="+i);
            doShow("frameDiv");
            doHide("listDiv");
        }
        ...
        //添加列表项
        function add(){
            $("#theFrame").attr("src","<%=basePath%>admin/ManageServlet?action=toAdd&lx=cb");
            doShow("frameDiv");
            doHide("listDiv");
        }
     	// 显示
        ...
        function hideAddFrame(){
             document.location.href='<%=basePath%>admin/ManageServlet?action=list&lx=cb';
        }
        ...
        function del(i){
            ...
            document.all.pageform.action='<%=basePath%>admin/ManageServlet?action=del&lx=cb'; 
            document.all.pageform.submit(); 
        }
        ...
        function goPage(i){
        	$("#pageNo").val(i);
      	    document.all.pageform.action='<%=basePath%>admin/ManageServlet?action=list&lx=cb';
		    document.all.pageform.submit();    
        }
  ...
  <body class="mainbody">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="msgtable">
			...
			<c:forEach items="${pager.resultList}" var="item">
		     <tr>
		        ...
		        <td>${item.yhm }</td>
		        <td>${item.xm }</td>
		        <td align="center"><a href="javascript:edit(${item.id })" target="_self">修改</a></td>
		      </tr>
		    </c:forEach>  
		</table>
      ...
      <div id="frameDiv" class="hideFrame" >
          <iframe  src="" width="100%" height="200" scrolling="auto" frameborder="0" name="theFrame" id="theFrame"></iframe>
      </div>
    </form>
    </body>
</html>