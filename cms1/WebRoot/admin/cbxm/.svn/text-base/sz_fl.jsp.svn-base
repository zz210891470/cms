		...
		function del(id){
			...
			if(confirm("确定要删除行业类型？")){
         		$.post('<%=basePath%>admin/IndustryServlet?act=del&id='+id, function(data){
	          	...
	          	if(data[0]=="SUCCESS"){
	          		location.href="<%=basePath%>admin/IndustryServlet?act=list&lx=cb";
	          	}
	    		});
			}
		}
  ...
  <body class="mainbody">
  	<div id="lb">
        <div class="select_box">
            ...
            <span style="float:right;"><a href="#" onclick="location.href='IndustryServlet?act=list&lx=cb';" target="_self">刷新</a></span>
        </div>
