package com.trunko.project.entity;

import java.sql.Date;

public class ProCbxm {

	private Long id; // 编号
	private Long nd; // 年度
	private String bh; // 项目编号
	private String mc; // 项目名称
	private String dq; // 地区
	private String jsgm; // 建设规模与内容
	private String jsdd; // 建设地点
	private Double ztz; // 总投资
	private String xyfx; // 经济效益分析
	private String sbdw; // 项目申报单位
	private String lxr; // 联系人
	private String lxdh; // 联系电话
	private String dzyx; // 电子邮箱
	private String chdw; // 项目策划单位
	private String zzdj; // 资质等级
	private String lxr1; // 联系人
	private String lxdh1; // 联系电话
	private String dzyx1; // 电子邮箱
	private String cbdw; // 项目承办单位
	private String lxr2; // 联系人
	private String lxdh2; // 联系电话
	private String dzyx2; // 电子邮箱
	private String hzfs; // 合作方式
	private String lygc; // 项目提出的理由与过程
	private String jstj; // 项目主要建设条件
	private String jsjjzb; // 主要技术经济指标
	private String tjzk; // 项目前期推进状况

	private String bgs; // 报告书类别
	private Date bssj; // 报送时间

	private String zt;
	private String sbyh;

	private String lb;
	private String lb1;
	private String lb2;
	private String lb3;

	private String shyj; // 审核意见

	public String getLb() {
		return lb;
	}

	public void setLb(String lb) {
		this.lb = lb;
	}

	public String getLb1() {
		return lb1;
	}

	public void setLb1(String lb1) {
		this.lb1 = lb1;
	}

	public String getLb2() {
		return lb2;
	}

	public void setLb2(String lb2) {
		this.lb2 = lb2;
	}

	public String getLb3() {
		return lb3;
	}

	public void setLb3(String lb3) {
		this.lb3 = lb3;
	}

	public String getBh() {
		return bh;
	}

	public void setBh(String bh) {
		this.bh = bh;
	}

	public String getMc() {
		return mc;
	}

	public void setMc(String mc) {
		this.mc = mc;
	}

	public String getDq() {
		return dq;
	}

	public void setDq(String dq) {
		this.dq = dq;
	}

	public String getJsgm() {
		return jsgm;
	}

	public void setJsgm(String jsgm) {
		this.jsgm = jsgm;
	}

	public String getJsdd() {
		return jsdd;
	}

	public void setJsdd(String jsdd) {
		this.jsdd = jsdd;
	}

	public String getXyfx() {
		return xyfx;
	}

	public void setXyfx(String xyfx) {
		this.xyfx = xyfx;
	}

	public String getSbdw() {
		return sbdw;
	}

	public void setSbdw(String sbdw) {
		this.sbdw = sbdw;
	}

	public String getLxr() {
		return lxr;
	}

	public void setLxr(String lxr) {
		this.lxr = lxr;
	}

	public String getLxdh() {
		return lxdh;
	}

	public void setLxdh(String lxdh) {
		this.lxdh = lxdh;
	}

	public String getChdw() {
		return chdw;
	}

	public void setChdw(String chdw) {
		this.chdw = chdw;
	}

	public String getLxr1() {
		return lxr1;
	}

	public void setLxr1(String lxr1) {
		this.lxr1 = lxr1;
	}

	public String getLxdh1() {
		return lxdh1;
	}

	public void setLxdh1(String lxdh1) {
		this.lxdh1 = lxdh1;
	}

	public String getBgs() {
		return bgs;
	}

	public void setBgs(String bgs) {
		this.bgs = bgs;
	}

	public Date getBssj() {
		return bssj;
	}

	public void setBssj(Date bssj) {
		this.bssj = bssj;
	}

	public void setSbyh(String sbyh) {
		this.sbyh = sbyh;
	}

	public String getSbyh() {
		return sbyh;
	}

	public void setZt(String zt) {
		this.zt = zt;
	}

	public String getZt() {
		return zt;
	}

	public void setShyj(String shyj) {
		this.shyj = shyj;
	}

	public String getShyj() {
		return shyj;
	}

	public Long getId() {
		return id;
	}

	public void setId(Long id) {
		this.id = id;
	}

	public Long getNd() {
		return nd;
	}

	public void setNd(Long nd) {
		this.nd = nd;
	}


	public Double getZtz() {
		return ztz;
	}

	public void setZtz(Double ztz) {
		this.ztz = ztz;
	}

	public String getDzyx() {
		return dzyx;
	}

	public void setDzyx(String dzyx) {
		this.dzyx = dzyx;
	}

	public String getZzdj() {
		return zzdj;
	}

	public void setZzdj(String zzdj) {
		this.zzdj = zzdj;
	}

	public String getDzyx1() {
		return dzyx1;
	}

	public void setDzyx1(String dzyx1) {
		this.dzyx1 = dzyx1;
	}

	public String getCbdw() {
		return cbdw;
	}

	public void setCbdw(String cbdw) {
		this.cbdw = cbdw;
	}

	public String getLxr2() {
		return lxr2;
	}

	public void setLxr2(String lxr2) {
		this.lxr2 = lxr2;
	}

	public String getLxdh2() {
		return lxdh2;
	}

	public void setLxdh2(String lxdh2) {
		this.lxdh2 = lxdh2;
	}

	public String getDzyx2() {
		return dzyx2;
	}

	public void setDzyx2(String dzyx2) {
		this.dzyx2 = dzyx2;
	}

	public String getHzfs() {
		return hzfs;
	}

	public void setHzfs(String hzfs) {
		this.hzfs = hzfs;
	}

	public String getLygc() {
		return lygc;
	}

	public void setLygc(String lygc) {
		this.lygc = lygc;
	}

	public String getJstj() {
		return jstj;
	}

	public void setJstj(String jstj) {
		this.jstj = jstj;
	}

	public String getJsjjzb() {
		return jsjjzb;
	}

	public void setJsjjzb(String jsjjzb) {
		this.jsjjzb = jsjjzb;
	}

	public String getTjzk() {
		return tjzk;
	}

	public void setTjzk(String tjzk) {
		this.tjzk = tjzk;
	}

}
