...
public class Uploader {
	...
	// 文件允许格式
	private String[] allowFiles = { ".rar", ".doc", ".docx", ".zip", ".pdf", ".xls" , ".xlsx",
	...
	public void upload() throws Exception {
		...
		try {
            ...
            File f = new File(this.getPhysicalPath(this.url));
            fileItem.write(f);
	...
	public void uploadBase64(String fieldName) {
		System.out.println("进入uploadbase64");
		String savePath = this.getFolder(this.savePath);
		String base64Data = this.request.getParameter(fieldName);
		this.fileName = this.getName("test.png");
		this.url = savePath + "/" + this.fileName;
		 System.out.println(base64Data+"fdgdsgsdfgsdfg");
	...
	private void parseParams() {
		System.out.println("获取文件扩展名");
	       DiskFileItemFactory dff = new DiskFileItemFactory();
	        try {
	            ServletFileUpload sfu = new ServletFileUpload(dff);
	            sfu.setSizeMax(this.maxSize);
	            sfu.setHeaderEncoding("UTF-8");//编码啊
	            List items = sfu.parseRequest(this.request);
	            ...
	            while (iter.hasNext())
	            {
	                ...
	                if (item.isFormField())
	                {
	                    this.params.put(item.getFieldName(), item.getString("UTF-8"));//编码啊
	                }
	                else {
	                    this.originalName = item.getName();
	                    // 只保留一个
	                    /*if (this.inputStream == null) {
	                        this.inputStream = ((FileItemStream)item).openStream();
	                        return;
	                    }
	                    */
	                    if(this.fileItem == null){
